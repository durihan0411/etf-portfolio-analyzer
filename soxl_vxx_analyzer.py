...
class SOXLVXXAnalyzer:
    def __init__(self):
        self.soxl_ticker = "SOXL"  # Direxion Daily Semiconductor Bull 3X ETF
        self.vxx_ticker = "VXX"    # iPath Series B S&P 500 VIX Short-Term Futures ETN
        self.soxl_data = None
        self.vxx_data = None
    ...
    def fetch_data(self, start_date=None, end_date=None):
        ...
        print("SOXL과 VXX 데이터를 수집하는 중...")
        
        if start_date is None:
            start_date = "2010-03-11"  # SOXL 상장일 (2010년 3월 11일)
        if end_date is None:
        ...
        try:
            # SOXL 데이터 수집
            print("SOXL 데이터 수집 중...")
            soxl_raw = yf.download(self.soxl_ticker, start=start_date, end=end_date, progress=False)
            if soxl_raw.empty:
            ...
            # MultiIndex 컬럼 처리
            if isinstance(soxl_raw.columns, pd.MultiIndex):
                soxl_raw.columns = soxl_raw.columns.droplevel(0)
            if isinstance(vxx_raw.columns, pd.MultiIndex):
                vxx_raw.columns = vxx_raw.columns.droplevel(0)
            
            # 컬럼명 정리
            if len(soxl_raw.columns) == 6:
                soxl_raw.columns = ['Open', 'High', 'Low', 'Close', 'Adj Close', 'Volume']
            elif len(soxl_raw.columns) == 5:
                soxl_raw.columns = ['Open', 'High', 'Low', 'Close', 'Volume']
            ...
            self.soxl_data = soxl_raw
            self.vxx_data = vxx_raw
            
            # 결합된 데이터
            self.combined_data = pd.DataFrame({
                'SOXL_Close': self.soxl_data['Close'],
                'VXX_Close': self.vxx_data['Close'],
                'SOXL_Volume': self.soxl_data['Volume'],
                'VXX_Volume': self.vxx_data['Volume']
            }).dropna()
            
            print(f"SOXL 데이터 수집 완료: {len(self.soxl_data)}개 거래일")
    ...
    def calculate_returns_and_metrics(self):
        ...
        # 일간 수익률 계산
        self.combined_data['SOXL_Return'] = self.combined_data['SOXL_Close'].pct_change()
        self.combined_data['VXX_Return'] = self.combined_data['VXX_Close'].pct_change()
        
        # 로그 수익률 계산
        self.combined_data['SOXL_Log_Return'] = np.log(self.combined_data['SOXL_Close'] / self.combined_data['SOXL_Close'].shift(1))
        self.combined_data['VXX_Log_Return'] = np.log(self.combined_data['VXX_Close'] / self.combined_data['VXX_Close'].shift(1))
        
        # 이동 평균 변동성 (20일, 60일)
        self.combined_data['SOXL_Volatility_20d'] = self.combined_data['SOXL_Return'].rolling(window=20).std() * np.sqrt(252)
        self.combined_data['VXX_Volatility_20d'] = self.combined_data['VXX_Return'].rolling(window=20).std() * np.sqrt(252)
        
        self.combined_data['SOXL_Volatility_60d'] = self.combined_data['SOXL_Return'].rolling(window=60).std() * np.sqrt(252)
        self.combined_data['VXX_Volatility_60d'] = self.combined_data['VXX_Return'].rolling(window=60).std() * np.sqrt(252)
        
        # 이동 상관관계 계산 (20일, 60일)
        self.combined_data['Correlation_20d'] = self.combined_data['SOXL_Return'].rolling(window=20).corr(self.combined_data['VXX_Return'])
        self.combined_data['Correlation_60d'] = self.combined_data['SOXL_Return'].rolling(window=60).corr(self.combined_data['VXX_Return'])
        
        # 연간화된 수익률과 변동성
        soxl_annual_return = self.combined_data['SOXL_Return'].mean() * 252
        vxx_annual_return = self.combined_data['VXX_Return'].mean() * 252
        
        soxl_annual_vol = self.combined_data['SOXL_Return'].std() * np.sqrt(252)
        vxx_annual_vol = self.combined_data['VXX_Return'].std() * np.sqrt(252)
        
        # 상관관계
        correlation = self.combined_data['SOXL_Return'].corr(self.combined_data['VXX_Return'])
        
        # 공분산 행렬
        returns_matrix = self.combined_data[['SOXL_Return', 'VXX_Return']].dropna()
        cov_matrix = returns_matrix.cov() * 252  # 연간화
        
        self.metrics = {
            'soxl_return': soxl_annual_return,
            'vxx_return': vxx_annual_return,
            'soxl_volatility': soxl_annual_vol,
    ...
    def analyze_correlation(self):
        ...
        # 전체 기간 상관관계
        overall_corr_price = self.combined_data['SOXL_Close'].corr(self.combined_data['VXX_Close'])
        overall_corr_return = self.combined_data['SOXL_Return'].corr(self.combined_data['VXX_Return'])
        ...
        for year in self.combined_data.index.year.unique():
            ...
            if len(year_data) > 20:  # 최소 20개 거래일 이상인 경우만
                price_corr = year_data['SOXL_Close'].corr(year_data['VXX_Close'])
                return_corr = year_data['SOXL_Return'].corr(year_data['VXX_Return'])
                yearly_corr.append({
                    'Year': year,
        ...
        # SOXL 레벨별 상관관계 분석
        soxl_levels = [10, 20, 30, 50, 70, 100, 150, 200]
        soxl_level_analysis = []
        
        for i in range(len(soxl_levels) - 1):
            low_soxl = soxl_levels[i]
            high_soxl = soxl_levels[i + 1]
            level_data = self.combined_data[
                (self.combined_data['SOXL_Close'] >= low_soxl) & 
                (self.combined_data['SOXL_Close'] < high_soxl)
            ]
            if len(level_data) > 20:
                price_corr = level_data['SOXL_Close'].corr(level_data['VXX_Close'])
                return_corr = level_data['SOXL_Return'].corr(level_data['VXX_Return'])
                soxl_level_analysis.append({
                    'SOXL_Range': f"{low_soxl}-{high_soxl}",
                    'Price_Correlation': price_corr,
                    'Return_Correlation': return_corr,
                    'Trading_Days': len(level_data),
                    'Avg_SOXL': level_data['SOXL_Close'].mean(),
                    'Avg_VXX': level_data['VXX_Close'].mean()
                })
        
        soxl_level_df = pd.DataFrame(soxl_level_analysis)
        
        # 통계적 유의성 검정
        t_stat, p_value = stats.pearsonr(
            self.combined_data['SOXL_Return'].dropna(),
        ...
        self.analysis_results = {
            ...
            'soxl_level_correlation': soxl_level_df,
            'data_summary': {
                ...
                'avg_soxl_price': self.combined_data['SOXL_Close'].mean(),
                'avg_vxx_price': self.combined_data['VXX_Close'].mean(),
                'soxl_volatility': self.combined_data['SOXL_Return'].std() * np.sqrt(252),
    ...
    def optimize_portfolio(self, risk_free_rate=0.03):
        ...
        returns = np.array([self.metrics['soxl_return'], self.metrics['vxx_return']])
        ...
        # 다양한 SOXL 비중으로 포트폴리오 계산 (0% ~ 100%)
        for soxl_weight in np.arange(0.0, 1.01, 0.01):
            vxx_weight = 1 - soxl_weight
            
            # 포트폴리오 수익률
            portfolio_return = soxl_weight * returns[0] + vxx_weight * returns[1]
            
            # 포트폴리오 변동성
            portfolio_variance = (soxl_weight**2 * cov_matrix[0,0] + 
                                vxx_weight**2 * cov_matrix[1,1] + 
                                2 * soxl_weight * vxx_weight * cov_matrix[0,1])
            portfolio_volatility = np.sqrt(portfolio_variance)
            
            # 샤프 비율
            sharpe_ratio = (portfolio_return - risk_free_rate) / portfolio_volatility
            
            portfolio_weights.append([soxl_weight, vxx_weight])
        ...
        portfolio_df = pd.DataFrame({
            'SOXL_Weight': [w[0] for w in portfolio_weights],
        ...
        # 특정 비중 조합들 분석
        specific_portfolios = {}
        for soxl_pct in [90, 80, 70, 60, 50]:
            soxl_weight = soxl_pct / 100
            vxx_weight = 1 - soxl_weight
            ...
            specific_portfolios[f'{soxl_pct}%_SOXL'] = {
                'SOXL_Weight': soxl_weight,
    ...
    def analyze_hedge_effectiveness(self):
        ...
        soxl_returns = self.combined_data['SOXL_Return'].dropna()
        vxx_returns = self.combined_data['VXX_Return'].dropna()
        
        for hedge_ratio in hedge_ratios:
            # 포트폴리오 수익률 (SOXL + VXX 헤지)
            portfolio_returns = soxl_returns + hedge_ratio * vxx_returns
    ...
    def create_visualizations(self):
        ...
        fig1 = make_subplots(
            rows=2, cols=1,
            subplot_titles=('SOXL vs VXX 가격 비교', 'SOXL과 VXX 수익률 비교'),
        ...
        fig1.add_trace(
            go.Scatter(
                ...
                y=self.combined_data['SOXL_Close'],
                name='SOXL Close',
        ...
        fig1.add_trace(
            go.Scatter(
                x=self.combined_data.index,
                y=self.combined_data['SOXL_Return'] * 100,
                name='SOXL Return (%)',
                line=dict(color='green', width=1),
                opacity=0.7
            ),
        ...
        fig1.update_layout(
            title='SOXL vs VXX 가격 및 수익률 비교',
            height=800,
            showlegend=True
        )
        
        fig1.update_yaxes(title_text="SOXL 가격 ($)", row=1, col=1)
        ...
        fig2.add_trace(
            go.Scatter(
                ...
                y=self.combined_data['SOXL_Return'].dropna() * 100,
                mode='markers',
                marker=dict(
                    size=4,
                ...
                hovertemplate='<b>%{text}</b><br>VXX 수익률: %{x:.2f}%<br>SOXL 수익률: %{y:.2f}%<extra></extra>'
            )
        )
        
        # 상관관계 선 추가
        vxx_returns = self.combined_data['VXX_Return'].dropna() * 100
        soxl_returns = self.combined_data['SOXL_Return'].dropna() * 100
        z = np.polyfit(vxx_returns, soxl_returns, 1)
        p = np.poly1d(z)
        ...
        fig2.update_layout(
            title='SOXL vs VXX 수익률 상관관계',
            xaxis_title='VXX 일간 수익률 (%)',
            yaxis_title='SOXL 일간 수익률 (%)',
            height=600
        )
        ...
        # 5. SOXL 레벨별 상관관계
        if not self.analysis_results['soxl_level_correlation'].empty:
            fig5 = go.Figure()
            
            fig5.add_trace(
                go.Bar(
                    x=self.analysis_results['soxl_level_correlation']['SOXL_Range'],
                    y=self.analysis_results['soxl_level_correlation']['Return_Correlation'],
                    name='SOXL 레벨별 상관계수',
                    marker_color='lightcoral',
                    text=self.analysis_results['soxl_level_correlation']['Trading_Days'],
                    textposition='outside'
                )
            )
            
            fig5.update_layout(
                title='SOXL 레벨별 SOXL-VXX 상관관계',
                xaxis_title='SOXL 레벨',
                yaxis_title='상관계수',
                height=500
            )
    ...
    def save_results(self, filename=None):
        ...
        if filename is None:
            ...
            filename = f"SOXL_VXX_analysis_{timestamp}.xlsx"
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            # SOXL 레벨별 상관관계
            if not self.analysis_results['soxl_level_correlation'].empty:
                self.analysis_results['soxl_level_correlation'].to_excel(writer, sheet_name='SOXL레벨별_상관관계', index=False)
            ...
            summary_data = {
                ...
                '값': [
                    ...
                    round(summary['avg_soxl_price'], 2),
                    round(summary['avg_vxx_price'], 2),
                    round(summary['soxl_volatility'], 4),
    ...
    def print_summary(self):
        ...
        print(f"평균 SOXL 가격: ${summary['avg_soxl_price']:.2f}")
        ...
        if self.portfolio_results:
            ...
            print(f"  SOXL 비중: {max_sharpe['SOXL_Weight']:.1%}")
            ...
            print(f"\n최소 변동성 포트폴리오:")
            print(f"  SOXL 비중: {min_vol['SOXL_Weight']:.1%}")
...
def main():
    """메인 실행 함수"""
    print("SOXL과 VXX 상관관계 분석을 시작합니다...")
    
    # 분석기 초기화
    analyzer = SOXLVXXAnalyzer()
    ...
    if figures:
        ...
        chart_names = [
            ...
            'rolling_correlation',
            'yearly_correlation',
            'soxl_level_correlation'
        ]
        
        for i, (fig, name) in enumerate(zip(figures, chart_names)):
            filename = f"SOXL_VXX_{name}_{timestamp}.html"
